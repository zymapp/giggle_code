...
## 🧩 3、支持故障转移与任务重做逻辑，保证不会有翻译任务被丢失。
   1️⃣ 故障转移（Failover）设计：定时心跳 + 掉线检测，每个 Worker 每 5s 上报一次心跳（worker_id, 时间戳），使用 Zookeeper Liveness-Probe 机制维护活跃节点，超过 30s 无心跳，则认为掉线，掉线的将不分配识别翻译任务
   2️⃣ 任务重做逻辑：用户提交记录先做持久化入库，标识记录状态，由异步 worket节点翻译成功后再标识成功状态，如果翻译失败将由各个 worker节点定时重新发起翻译处理
